...
PostsDAO.prototype.LatestPosts = function (callback) {
    ...
    "INNER JOIN Pillars d ON (d.Id=c.Pillars_Id) LIMIT 3", callback);
}

PostsDAO.prototype.AllPosts = function (callback) {
    this._connection.query("SELECT a.Id, a.Title, a.Text, a.DateUpdated AS Date, b.Title as Theme, c.Title as Category, u.Email "+
    "FROM Posts a INNER JOIN Themes b ON (a.Themes_Id=b.Id)" + 
    "INNER JOIN User u ON (a.UserId=u.Id)" + 
    "INNER JOIN Knowledge c ON (c.Id=b.Knowledge_Id)" + 
    "INNER JOIN Pillars d ON (d.Id=c.Pillars_Id)", callback);
}
